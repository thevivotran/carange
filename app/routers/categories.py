...
def create_category(category: CategoryCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(Category).filter(Category.name == category.name, Category.type == category.type).first()
...
def update_category(category_id: int, category: CategoryUpdate, db: Session = Depends(get_db)):
    ...
    if category.name != db_category.name:
        existing = (
            db.query(Category)
            .filter(Category.name == category.name, Category.type == category.type, Category.id != category_id)
            .first()
        )
