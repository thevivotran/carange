...
def cat_ids(client):
    ...
    inc = client.post(
        "/api/categories/", json={"name": "Salary", "type": "income", "color": "#green", "icon": "x"}
    ).json()["id"]
    exp = client.post(
        "/api/categories/", json={"name": "Food", "type": "expense", "color": "#red", "icon": "x"}
    ).json()["id"]
...
def test_create_income_transaction(client, cat_ids):
    r = _make_tx(client, date_str="2026-04-01", amount=5_000_000, type_="income", category_id=cat_ids["income"])
...
def test_create_expense_transaction(client, cat_ids):
    r = _make_tx(client, date_str="2026-04-02", amount=300_000, type_="expense", category_id=cat_ids["expense"])
    assert r.status_code == 200
    assert r.json()["type"] == "expense"


def test_create_transaction_zero_amount_rejected(client, cat_ids):
    r = _make_tx(client, date_str="2026-04-01", amount=0, type_="income", category_id=cat_ids["income"])
    assert r.status_code == 422


def test_create_transaction_negative_amount_rejected(client, cat_ids):
    r = _make_tx(client, date_str="2026-04-01", amount=-500, type_="income", category_id=cat_ids["income"])
    assert r.status_code == 422
...
def test_get_single_transaction(client, cat_ids):
    tx_id = _make_tx(client, date_str="2026-04-05", amount=9_000, type_="income", category_id=cat_ids["income"]).json()[
        "id"
    ]
...
def test_update_transaction_amount(client, cat_ids):
    tx_id = _make_tx(client, date_str="2026-04-01", amount=1_000, type_="income", category_id=cat_ids["income"]).json()[
        "id"
    ]
...
def test_update_transaction_date(client, cat_ids):
    tx_id = _make_tx(client, date_str="2026-04-01", amount=500, type_="expense", category_id=cat_ids["expense"]).json()[
        "id"
    ]
